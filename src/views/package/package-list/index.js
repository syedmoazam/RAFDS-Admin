...
const PackageList = () => {
    ...
    const handleFilter = e => {
        ...
        if (value.length) {
            updatedData = packageList.filter(item => {
                const startsWith =
    ...
    const handleModal = (getPackages = null) => {
        if(getPackages) fetchPackages();
    ...
    const fetchPackages = () => {
        axios.get(environment.SERVER_BASE_URL + '/admin/packages/all').then(res => {
            const packages = res.data.map((packageInfo, index) => ({...packageInfo.packageRecord, id: index+1}));
        ...
        <Fragment>
            ...
            <Card>
                ...
                <DataTable
                    ...
                    data={searchValue.length ? filteredData : packageList}
                />
            </Card>
